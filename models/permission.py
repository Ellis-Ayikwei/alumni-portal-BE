...
"""
@app.route('/alumni/api/v1/alumni_groups', methods=['POST'])
@require_permission(ResourceType.ALUMNI_GROUP.value, Action.ADD.value)
def ADD_alumni_group():
    # Only users with ADD permission for ALUMNI_GROUP can access
    pass

@app.route('/alumni/api/v1/contracts/<contract_id>', methods=['PUT'])
@require_permission(ResourceType.CONTRACT.value, Action.CHANGE.value)
def CHANGE_contract(contract_id):
    # Only users with CHANGE permission for CONTRACT can access
    pass
"""